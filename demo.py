...
# maximize action based on reward achieved in agent envriroment loop.
# this is called Q Learning.
# based on just game state / images. mapping of state to action is known as 'policy'
...
def updateBall(paddle1YPos, paddle2YPos, ballXPos, ballYPos, ballXDirection, ballYDirection):
    ...
    if (ballYPos <= 0):
        ballYPos = 0
        ballYDirection = 1 # simulates bouncing off roof
    # if it hits the bottom, move up
    elif (ballYPos >= WINDOW_HEIGHT - BALL_HEIGHT):
        ballYPos = WINDOW_HEIGHT - BALL_HEIGHT
        ballYDirection = -1 # simulates bouncing off floor
    return [score, paddle1YPos, paddle2YPos, ballXPos, ballYPos, ballXDirection, ballYDirection]
...
class PongGame:
    ...
    def getNextFrame(self, action):
        ...
        [score, self.paddle1YPos, self.paddle2YPos, self.ballXPos, self.ballYPos, self.ballXDirection,
         self.ballYDirection] = updateBall(self.paddle1YPos, self.paddle2YPos, self.ballXPos, self.ballYPos,
